...
class Account < ApplicationRecord
  ...
  def on_tweet_changed(tweet)
    subscribers.each { |sub| Broadcast::Tweet.replace(account: self, tweet:, subscriber: sub) }
  end

  def on_tweet_published(tweet)
    subscribers.each { |sub| Broadcast::Tweet.prepend(account: self, tweet:, subscriber: sub) }
  end
